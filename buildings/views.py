...
def getTicket(request):
    ...
    if ticket_no is not None:
        ...
        for t in range(total_department):
            department = Department.objects.all().filter(
                id=serializer.data[0]["department"][t]
            )
        ...
        for i in range(len(serializer.data[0]["agents_assigned"])):
            assignee = Assignee.objects.all().filter(
                id=serializer.data[0]["agents_assigned"][i]
            )
            serialiser = AssigneeSerializer(assignee, many=True)
            if serialiser.data[0]["status"] == "Completed":
                activity = Activity.objects.values().filter(
                    ticket_id=serializer.data[0]["id"]
                )
